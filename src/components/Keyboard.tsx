...
import { backspace, enter, getKeyboardColors, hasGameEnded, pressKey } from "../game.ts"
...
const Key: Component<{ key: string }> = (props) => {
	const getKeyStyle = () => {
...
export const Keyboard: Component = () => {
	return (
		<div class=":uno: grid-(~ cols-20 auto-rows-15 gap-1.5) [touch-action:manipulation]">
			{[...QWERTY].map(
				(key) => (key === "+" ? <EnterKey /> : key === "-" ? <BackSpaceKey /> : <Key key={key} />)
			)}
		</div>
	)
}
